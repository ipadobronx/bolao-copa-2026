  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <header className="bg-bg-dark/95 border-border fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b px-4 backdrop-blur-md md:hidden">
        <span className="font-display text-danger flex items-center gap-2 text-xl tracking-[2px]">
          <span
            aria-hidden="true"
            className="bg-danger text-bg-dark flex h-7 w-7 -rotate-[5deg] items-center justify-center rounded-md text-base font-black"
          >
            A
          </span>
          <span>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 left-0 z-50 w-72 outline-none md:hidden">
          <Dialog.Title className="sr-only">Menu admin</Dialog.Title>
          <Dialog.Description className="sr-only">Navegação do painel admin</Dialog.Description>
